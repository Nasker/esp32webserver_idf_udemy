...
esp_err_t app_nvs_save_sta_creds(void)
{
	...
	if (wifi_sta_config)
	{
		...
		if (esp_err != ESP_OK)
		{
			ESP_LOGE(TAG, "app_nvs_save_sta_creds: Error (%s) opening NVS handle!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
		...
		if (esp_err != ESP_OK)
		{
			ESP_LOGE(TAG, "app_nvs_save_sta_creds: Error (%s) comitting credentials to NVS!\n", esp_err_to_name(esp_err));
			return esp_err;
		}
		nvs_close(handle);
...
bool app_nvs_load_sta_creds(void)
{
	...
	if (nvs_open(app_nvs_sta_creds_namespace, NVS_READONLY, &handle) == ESP_OK)
	{
		...
		{
			free(wifi_config_buff);
			ESP_LOGI(TAG, "app_nvs_load_sta_creds: (%s) retrieving password!\n", esp_err_to_name(esp_err));
			return false;
		}
		...
		ESP_LOGI(TAG, "app_nvs_load_sta_creds: SSID: %s Password: %s\n", wifi_sta_config->sta.ssid, wifi_sta_config->sta.password);
